...
void main(void)
{
	...
	EM gmm(num_clusters, cov_mat_type, term);
	
	// Create the feature matrix, as an 𝐻𝑊 by 𝐷 Mat object
	Mat feature(img.rows * img.cols, 3, CV_64F);
	...
	Mat initialMeans(num_clusters, 3, CV_64F);
	...
	for (int y = 0; y < img.rows; y++) {
		for (int x = 0; x < img.cols; x++) {
			int result = (int)gmm.predict(feature.row(index++))[1];
			segmented[result].at<Point3d>(y, x, 0) = float_img.at<Point3d>(y, x, 0);
		}
	}
	imshow("clustered image", segmented[0]);
	waitKey(0);
}